...
def run_fun_class(config: Dict[str, Any], log_callback: Callable[[str], None], stop_flag: Callable[[], bool]):
    """
    执行通识选修补选监控（无限轮询，发现空位即抢，仅抢一次）
    ...
    :param config: 配置字典，包含以下字段：
        ...
        【注意】原脚本使用 WaitTime 作为轮询间隔，
        GUI 在调用时传入的是 BetweenTime，此处兼容处理。
    ...
    # 兼容 GUI 传入的 "BetweenTime"（补选/通识 Tab 使用此名）
            ...
            if response.status_code == 200:
                try:
                    ...
                    if not rows:
                        ...
                        return
                    # 注意：Fun_Class.py 原始逻辑是直接取 rows[0]，不经过 tcList
                    teaching_class = rows[0]
    ...
    # ===== 7. 主循环 =====
    log_callback("🔍 开始监控通识课程余量，发现空位自动抢课...")
    while True:
        if stop_flag():
